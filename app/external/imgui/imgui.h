...
namespace ImGui
{
    ...
    EndFrame(); // ends the Dear ImGui frame. automatically called by Render().
                ...
                // call EndFrame() without Render()... but you'll have wasted
                // CPU already! If you don't need to render, better to not
    ...
    IMGUI_API void ShowUserGuide(); // add basic help/info block (not a window):
                                    // how to manipulate ImGui as an end-user
    ...
    IMGUI_API bool IsWindowHovered(
        ImGuiHoveredFlags flags =
            0); // is current window hovered and hoverable (e.g. not blocked by
                ...
                // should be dispatched to Dear ImGui or to your underlying app,
                // you should not use this function! Use the
    ...
    GetWindowPos(); // get current window position in screen space (note: it is
                    // unlikely you need to use this. Consider using current
                    // layout pos instead, GetCursorScreenPos())
    IMGUI_API ImVec2
    GetWindowSize(); // get current window size (note: it is unlikely you need
                     // to use this. Consider using GetCursorScreenPos() and
                     // e.g. GetContentRegionAvail() instead)
    ...
    SetWindowSize(const ImVec2 &size,
                  ImGuiCond cond =
                      0); // (not recommended) set current window size - call
    ...
    // incomplete and the Min/Max return values are in local window coordinates
    ...
    //    - Window-local coordinates:   SameLine(), GetCursorPos(),
    //    SetCursorPos(), GetCursorStartPos(), GetContentRegionMax(),
    //    GetWindowContentRegion*(), PushTextWrapPos()
    ...
    // coordinates.
    IMGUI_API ImVec2
    GetCursorScreenPos(); // cursor position in absolute coordinates (prefer
                          // using this, also more useful to work with
                          // ImDrawList API).
    ...
    IMGUI_API ImVec2 GetCursorStartPos(); // [window-local] initial cursor
    ...
    EndGroup(); // unlock horizontal starting position + capture the whole group
                // bounding box into one "item" (so you can use IsItemHovered()
    ...
    GetTextLineHeightWithSpacing(); // ~ FontSize + style.ItemSpacing.y
                                    // (distance in pixels between 2 consecutive
                                    // lines of text)
    ...
    IMGUI_API bool
    SmallButton(const char *label); // button with (FramePadding.y == 0) to
    ...
    // link above.
    // - Note that Image() may add +2.0f to provided size if a border is
    // visible, ImageButton() adds style.FramePadding*2.0f to provided size.
    ...
    SliderFloat(const char *label, float *v, float v_min, float v_max,
                ...
                ImGuiSliderFlags flags =
                    0); // adjust format to decorate the value with a prefix or
    ...
    ColorButton(const char *desc_id, const ImVec4 &col,
                ImGuiColorEditFlags flags = 0,
                const ImVec2 &size =
                    ImVec2(0, 0)); // display a color square/button, hover for
    ...
    GetTreeNodeToLabelSpacing(); // horizontal distance preceding label when
                                 // using TreeNode*() or Bullet() == (g.FontSize
                                 // + style.FramePadding.x*2) for a regular
                                 // unframed TreeNode
    ...
    IMGUI_API bool
    Selectable(const char *label, bool *p_selected,
               ImGuiSelectableFlags flags = 0,
               const ImVec2 &size =
    ...
    TableNextColumn(); // append into the next column (or first column of next
                       ...
                       // column is visible.
    IMGUI_API bool
    TableSetColumnIndex(int column_n); // append into the specified column.
    ...
    IMGUI_API void
    TableHeadersRow(); // submit a row with headers cells based on data provided
                       // to TableSetupColumn() + submit context menu
    IMGUI_API void
    TableAngledHeadersRow(); // submit a row with angled headers
    ...
    TabItemButton(const char *label,
                  ImGuiTabItemFlags flags =
                      0); // create a Tab behaving like a button. return true
    ...
    IMGUI_API bool SetDragDropPayload(
        ...
        ImGuiCond cond =
            0); // type is a user defined string of maximum 32 characters.
                ...
                // internal types. Data is copied and held by imgui. Return true
    ...
    SetNextItemAllowOverlap(); // allow next item to be overlapped by a
                               ...
                               // buttons, selectable, treenode covering an area
                               // where subsequent items may need to be added.
                               // Note that both Selectable() and TreeNode()
    ...
    IMGUI_API bool
    IsItemEdited(); // did the last item modify its underlying value this frame?
                    // or was pressed? This is generally the same as the "bool"
    ...
    IMGUI_API const char *
    GetStyleColorName(ImGuiCol idx); // get a string corresponding to the enum
    ...
    // - before v1.87, we used ImGuiKey to carry native/user indices as defined
    // by each backends. About use of those legacy ImGuiKey values:
    //  - without IMGUI_DISABLE_OBSOLETE_KEYIO (legacy support): you can still
    //  use your legacy native/user indices (< 512) according to how your
    //  backend/engine stored them in io.KeysDown[], but need to cast them to
    //  ImGuiKey.
    //  - with    IMGUI_DISABLE_OBSOLETE_KEYIO (this is the way forward): any
    //  use of ImGuiKey will assert with key < 512. GetKeyIndex() is
    //  pass-through and therefore deprecated (gone if
    ...
    IMGUI_API bool
    IsMouseReleased(ImGuiMouseButton button); // did mouse button released?
    ...
    IsMousePosValid(const ImVec2 *mouse_pos =
                        NULL); // by convention we use (-FLT_MAX,-FLT_MAX) to
    ...
    IsAnyMouseDown(); // [WILL OBSOLETE] is any mouse button held? This was
                      ...
                      // maintain a mask of held mouse buttons, because upcoming
    ...
    IMGUI_API ImVec2 GetMouseDragDelta(
        ...
        float lock_threshold =
            -1.0f); // return the delta from the initial clicking position while
                    ...
                    // distance threshold at least once (uses
    ...
    GetMouseCursor(); // get desired mouse cursor shape. Important: reset in
                      ...
                      // valid before Render(). If you use software rendering by
                      // setting io.MouseDrawCursor ImGui will render those for
                      // you
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_SpanAllColumns =
        1 << 14, // Frame will span all columns of its container table (text
                 // will still fit in current column)
    ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
        1
        << 15, // (WIP) Nav: left direction may move to this TreeNode() from any
...
namespace ImGui
{
    ...
    ImageButton(ImTextureID user_texture_id, const ImVec2 &size,
                ...
                const ImVec4 &tint_col = ImVec4(
                    1, 1, 1, 1)); // Use new ImageButton() signature (explicit
    ...
    // // Renamed as name was misleading + removed default value. static inline
    // void  CaptureMouseFromApp(bool want_capture_mouse = true)               {
    ...
    // ImGuiSliderFlags_Logarithmic. See github.com/ocornut/imgui/issues/3361
    // for details. IMGUI_API bool      DragScalar(const char* label,
    // ImGuiDataType
