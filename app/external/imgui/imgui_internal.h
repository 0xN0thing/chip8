...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_Inputable =
        1 << 10, // false     // [WIP] Auto-activate input mode when tab
...
namespace ImGui
{
    ...
    IMGUI_API void Shutdown(); // Since 1.60 this is a _private_ function. You
    ...
    // FocusItemByID(id), ActivateItem(offset = -1), ActivateItemByID(id) etc.
    // which are much harder to design and implement than expected. I have a
    // couple of private branches on this matter but it's not simple. For now
    ...
    IMGUI_API void ActivateItemByID(
        ImGuiID id); // Activate an item by ID (button, checkbox, tree node
                     ...
                     // frame when the item is encountered again.
    ...
    // - The idea is that instead of "eating" a given input, we can link to an
    // owner id.
    // - Ownership is most often claimed as a result of reacting to a press/down
    // event (but occasionally may be claimed ahead).
    // - Input queries can then read input by specifying ImGuiKeyOwner_Any (==
    // 0), ImGuiKeyOwner_NoOwner (== -1) or a custom ID.
    ...
    // released. Therefore:
    //   - for ownership registration happening as a result of a down/press
    //   event, the SetKeyOwner() call may be done once (common case).
    //   - for ownership registration happening ahead of a down/press event, the
    //   SetKeyOwner() call needs to be made every frame (happens if e.g.
    //   claiming ownership on hover).
    // - SetItemKeyOwner() is a shortcut for common simple case. A custom widget
    // will probably want to call SetKeyOwner() multiple times directly based on
    // its interaction state.
    // - This is marked experimental because not all widgets are fully honoring
    // the Set/Test idioms. We will need to move forward step by step.
    //   Please open a GitHub Issue to submit your usage scenario or if there's
    //   a use case you need solved.
    ...
    // [EXPERIMENTAL] High-Level: Input Access functions w/ support for
    // Key/Input Ownership
    // - Important: legacy IsKeyPressed(ImGuiKey, bool repeat=true) _DEFAULTS_
    // to repeat, new IsKeyPressed() requires _EXPLICIT_ ImGuiInputFlags_Repeat
    // flag.
    // - Expected to be later promoted to public API, the prototypes are
    // designed to replace existing ones (since owner_id can default to Any ==
    // 0)
    // - Specifying a value for 'ImGuiID owner' will test that EITHER the key is
    // NOT owned (UNLESS locked), EITHER the key is owned by 'owner'.
    //   Legacy functions use ImGuiKeyOwner_Any meaning that they typically
    //   ignore ownership, unless a call to SetKeyOwner() explicitly used
    //   ImGuiInputFlags_LockThisFrame or ImGuiInputFlags_LockUntilRelease.
    // - Binding generators may want to ignore those for now, or suffix them
    // with Ex() until we decide if this gets moved into public API.
    ...
    // - When a policy (except for ImGuiInputFlags_RouteAlways *) is set,
    // Shortcut() will register itself with SetShortcutRouting(),
    ...
    //   window is in focus stack. Deep-most focused window takes inputs.
    //   ActiveId takes inputs over deep-most focused window.)
    //   - Routes are requested given a chord (key + modifiers) and a routing
    //   policy.
    //   - Routes are resolved during NewFrame(): if keyboard modifiers are
    //   matching current ones: SetKeyOwner() is called + route is granted for
    //   the frame.
    //   - Each route may be granted to a single owner. When multiple requests
    //   are made we have policies to select the winning route (e.g. deep most
    //   window).
    //   - Multiple read sites may use the same owner id can all access the
    //   granted route.
    ...
    IMGUI_API bool TestShortcutRouting(ImGuiKeyChord key_chord,
                                       ImGuiID owner_id);
    ...
    // - Selection patterns generally need to react (e.g. clear a selection)
    // when landing on one item of the set.
    //   So in order to identify a set multiple lists in same window may each
    //   need a focus scope. If you imagine an hypothetical
    //   BeginSelectionGroup()/EndSelectionGroup() api, it would likely call
    //   PushFocusScope()/EndFocusScope()
    // - Shortcut routing also use focus scope as a default location identifier
    // if an owner is not provided. We don't use the ID Stack for this as it is
    // common to want them separate.
    ...
    IMGUI_API void
    TableSetColumnSortDirection(int column_n, ImGuiSortDirection sort_direction,
    ...
    IMGUI_API ImGuiTabItem *TabBarFindTabByID(ImGuiTabBar *tab_bar,
                                              ImGuiID tab_id);
    IMGUI_API ImGuiTabItem *TabBarFindTabByOrder(ImGuiTabBar *tab_bar,
                                                 int order);
    ...
    IMGUI_API ImGuiID GetWindowResizeCornerID(ImGuiWindow *window,
                                              int n); // 0..3: corners
    IMGUI_API ImGuiID GetWindowResizeBorderID(ImGuiWindow *window,
                                              ImGuiDir dir);
    ...
    IMGUI_API void DebugLocateItemOnHover(
        ImGuiID target_id); // Only call on reaction to a mouse Hover: because
    ...
    // Refactored focus/nav/tabbing system in 1.82 and 1.84. If you have
    // old/custom copy-and-pasted widgets which used FocusableItemRegister():
    //  (Old) IMGUI_VERSION_NUM  < 18209: using 'ItemAdd(....)' and 'bool
    //  tab_focused = FocusableItemRegister(...)' (Old) IMGUI_VERSION_NUM >=
    //  18209: using 'ItemAdd(..., ImGuiItemAddFlags_Focusable)'  and 'bool
    //  tab_focused = (g.LastItemData.StatusFlags &
    //  ImGuiItemStatusFlags_Focused) != 0' (New) IMGUI_VERSION_NUM >= 18413:
    //  using 'ItemAdd(..., ImGuiItemFlags_Inputable)'     and 'bool tab_focused
