...
static char *
getLastName(char *const aoDestination, /* make sure it is allocated */
...
static int replaceChr(char *const aString, char const aOldChr,
                      char const aNewChr)
{
    char *p;
    int lRes = 0;
...
static wchar_t *getPathWithoutFinalSlashW(
    ...
    wchar_t const
        *const aSource) /* aoDestination and aSource can be the same */
{
    wchar_t const *lTmp;
    if (aSource)
    {
...
static wchar_t *
getLastNameW(wchar_t *const aoDestination, /* make sure it is allocated */
...
static void RGB2HexW(unsigned char const aRGB[3], wchar_t aoResultHexRGB[8])
{
    if (aoResultHexRGB)
    {
        if (aRGB)
        {
            ...
            swprintf(aoResultHexRGB, L"#%02hhx%02hhx%02hhx", aRGB[0], aRGB[1],
                     aRGB[2]);
#else
            swprintf(aoResultHexRGB, 8, L"#%02hhx%02hhx%02hhx", aRGB[0],
                     aRGB[1], aRGB[2]);
#endif
...
static void runSilentA(char const *const aString)
{
    ...
    if (!CreateProcessA(NULL, lArgs, NULL, NULL, FALSE, CREATE_NEW_CONSOLE,
...
static void runSilentW(wchar_t const *const aString)
{
    ...
    if (!CreateProcessW(NULL, lArgs, NULL, NULL, FALSE, CREATE_NEW_CONSOLE,
                        NULL, NULL, &StartupInfo, &ProcessInfo))
    {
        free(lArgs);
        return; /* GetLastError(); */
...
int tinyfd_messageBoxW(
    ...
    int const aDefaultButton) /* 0 for cancel/no , 1 for ok/yes , 2 for no in
                                 yesnocancel */
{
    int lBoxReturnValue;
    UINT aCode;
...
static char const *inputBoxWinGui(
    char *const aoBuff, char const *const aTitle, /* NULL or "" */
    ...
    char const *const aDefaultInput) /* "" , if NULL it's a passwordBox */
{
    ...
    lDialogString =
        (char *)malloc(3 * MAX_PATH_OR_CMD + lTitleLen + lMessageLen);
    ...
    wchar_t const *const aSingleFilterDescription) /* NULL or "image files" */
{
    ...
    ofn.hInstance = 0;
    ofn.lpstrFilter =
        lFilterPatterns && wcslen(lFilterPatterns) ? lFilterPatterns : NULL;
    ...
    char const *const aSingleFilterDescription) /* NULL or "image files" */
{
    ...
    lTmpWChar = tinyfd_saveFileDialogW(lTitle, lDefaultPathAndFile,
...
#ifndef TINYFD_NOSELECTFOLDERWIN
static int __stdcall BrowseCallbackProc(HWND hwnd, UINT uMsg, LPARAM lp,
                                        LPARAM pData)
{
...
wchar_t const *
tinyfd_selectFolderDialogW(wchar_t const *const aTitle,       /*  NULL or "" */
...
static char const *
selectFolderDialogWinGui8(char *const aoBuff,
                          char const *const aTitle,       /*  NULL or "" */
...
wchar_t const *
tinyfd_colorChooserW(wchar_t const *const aTitle,         /* NULL or "" */
...
static char const *
colorChooserWinGui8(char const *const aTitle,           /* NULL or "" */
    ...
    char const *const aSingleFilterDescription) /* NULL or "image files" */
{
    ...
    ofn.hInstance = 0;
    ofn.lpstrFilter =
        lFilterPatterns && strlen(lFilterPatterns) ? lFilterPatterns : NULL;
    ofn.lpstrCustomFilter = NULL;
    ofn.nMaxCustFilter = 0;
    ofn.nFilterIndex = 1;
...
#ifndef TINYFD_NOSELECTFOLDERWIN
static char const *
selectFolderDialogWinGuiA(char *const aoBuff,
                          char const *const aTitle,       /*  NULL or "" */
                          char const *const aDefaultPath) /* NULL or "" */
{
    BROWSEINFOA bInfo;
...
static char const *
colorChooserWinGuiA(char const *const aTitle,           /* NULL or "" */
                    char const *const aDefaultHexRGB,   /* NULL or "#FF0000"*/
                    unsigned char const aDefaultRGB[3], /* { 0 , 255 , 255 } */
                    unsigned char aoResultRGB[3])       /* { 0 , 0 , 0 } */
{
...
static char const *
saveFileDialogWinConsole(char *const aoBuff,
                         char const *const aTitle,              /* NULL or "" */
                         char const *const aDefaultPathAndFile) /* NULL or "" */
{
    char lDialogString[MAX_PATH_OR_CMD];
...
static char const *
selectFolderDialogWinConsole(char *const aoBuff,
                             char const *const aTitle,       /*  NULL or "" */
                             char const *const aDefaultPath) /* NULL or "" */
{
    char lDialogString[MAX_PATH_OR_CMD];
                                 ...
                                 yesnocancel */
{
        ...
        (!getenv("SSH_CLIENT") || getenv("DISPLAY")))
    {
        ...
        if (tinyfd_winUtf8)
        {
            return messageBoxWinGui8(aTitle, aMessage, aDialogType, aIconType,
                                     aDefaultButton);
        }
        else
        {
            return messageBoxWinGuiA(aTitle, aMessage, aDialogType, aIconType,
                                     aDefaultButton);
        }
    }
    else
        ...
        if (dialogPresent())
        {
            ...
            return messageBoxWinConsole(aTitle, aMessage, aDialogType,
    ...
    char const *const aSingleFilterDescription) /* NULL or "image files" */
{
        ...
        (!getenv("SSH_CLIENT") || getenv("DISPLAY")))
    {
        ...
        if (tinyfd_winUtf8)
        {
            p = saveFileDialogWinGui8(lBuff, aTitle, aDefaultPathAndFile,
                                      aNumOfFilterPatterns, aFilterPatterns,
                                      aSingleFilterDescription);
        }
        else
        {
            p = saveFileDialogWinGuiA(lBuff, aTitle, aDefaultPathAndFile,
                                      aNumOfFilterPatterns, aFilterPatterns,
                                      aSingleFilterDescription);
        }
    }
    ...
    int const aAllowMultipleSelects)            /* 0 or 1 */
{
        ...
        (!getenv("SSH_CLIENT") || getenv("DISPLAY")))
    {
        ...
        if (tinyfd_winUtf8)
        {
            p = openFileDialogWinGui8(lBuff, aTitle, aDefaultPathAndFile,
                                      aNumOfFilterPatterns, aFilterPatterns,
                                      aSingleFilterDescription,
        ...
        {
            p = openFileDialogWinGuiA(lBuff, aTitle, aDefaultPathAndFile,
                                      aNumOfFilterPatterns, aFilterPatterns,
                                      aSingleFilterDescription,
                                      aAllowMultipleSelects);
        }
        ...
        if (dialogPresent())
        {
            ...
            p = openFileDialogWinConsole(lBuff, aTitle, aDefaultPathAndFile,
                                         aAllowMultipleSelects);
        }
        else
        {
...
char const *
tinyfd_selectFolderDialog(char const *const aTitle,       /*  NULL or "" */
                          char const *const aDefaultPath) /* NULL or "" */
{
    static char lBuff[MAX_PATH_OR_CMD];
    char const *p;
...
char const *
tinyfd_colorChooser(char const *const aTitle,           /* NULL or "" */
                    char const *const aDefaultHexRGB,   /* NULL or "#FF0000"*/
                    unsigned char const aDefaultRGB[3], /* { 0 , 255 , 255 } */
                    unsigned char aoResultRGB[3])       /* { 0 , 0 , 0 } */
{
        ...
        (!getenv("SSH_CLIENT") || getenv("DISPLAY")))
    {
        ...
        if (tinyfd_winUtf8)
        {
            return colorChooserWinGui8(aTitle, aDefaultHexRGB, aDefaultRGB,
                                       aoResultRGB);
        }
        else
        {
            return colorChooserWinGuiA(aTitle, aDefaultHexRGB, aDefaultRGB,
                                       aoResultRGB);
        }
    }
    else
...
static char const *
getVersion(char const *const aExecutable) /*version # must follow :*/
{
    static char lBuff[MAX_PATH_OR_CMD];
...
static int isTerminalRunning() { return isatty(1); }
...
static char const *terminalName()
{
    ...
    if (lTerminalName[0] == '*')
    {
        ...
        /*else if ( strcpy(lTerminalName,"gnome-terminal") //bad (good if
        version < 3)
                  && detectPresence(lTerminalName) )
        {
            ...
            !(lTkinter2Present = tryCommand(lPythonCommand)))
        {
            ...
            else
            {
                for (i = 9; i >= 0; i--)
                {
                    ...
                    if (detectPresence(gPython2Name))
                    {
                        sprintf(lPythonCommand, "%s %s", gPython2Name,
                                lPythonParams);
                        lTkinter2Present = tryCommand(lPythonCommand);
                        break;
                    }
                                 ...
                                 yesnocancel */
{
    ...
    if (osascriptPresent())
    {
        ...
        strcat(lDialogString, "-e 'if vButton is \"Yes\" then' -e 'return 1' "
                              "-e 'else if vButton is \"No\" then' -e 'return "
                              "2' -e 'else' -e 'return 0' -e 'end if' ");
    ...
    else if (zenityPresent() || matedialogPresent() || qarmaPresent())
    {
        ...
        if (zenity3Present() >= 3)
        {
            ...
            if (aIconType &&
                (!strcmp("question", aIconType) ||
                 !strcmp("error", aIconType) || !strcmp("warning", aIconType)))
            {
...
to set mycolor to choose color default color {");
    ...
    else if (zenity3Present() || matedialogPresent() || qarmaPresent())
    {
        ...
        if (zenity3Present())
        {
            ...
            sprintf(lDialogString,
                    "zenity --color-selection --show-palette --color=%s",
                    lpDefaultHexRGB);
        }
        else if (matedialogPresent())
        {
            ...
            sprintf(lDialogString,
                    "matedialog --color-selection --show-palette --color=%s",
                    lpDefaultHexRGB);
        }
        else
        {
            ...
            sprintf(lDialogString,
                    "qarma --color-selection --show-palette --color=%s",
                    lpDefaultHexRGB);
        }
    ...
    else if (kdialogPresent())
    {
        ...
        sprintf(lDialogString, "kdialog --getcolor --default '%s'",
                lpDefaultHexRGB);
        if (aTitle && strlen(aTitle))
        {
            strcat(lDialogString, " --title \"");
